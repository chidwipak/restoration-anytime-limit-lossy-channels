...
### 2.3.4 Derivation of the rate condition (R)
$R(1-p)\ge h_R$ follows from Matveev–Pogromsky necessity applied to the **time-average** delivered rate. *Why the average is the right object:* by the SLLN for i.i.d. erasures, over $n$ steps the delivered nats $=nR(1-p)(1+o(1))$ a.s.; the restoration-entropy counting requires $\ge n\,h_R(1+o(1))$ nats to keep the volume of the uncertainty set bounded (Lemma C volume branch + isodiametric). If $R(1-p)<h_R$, the average information deficit grows linearly, so even **a.s.** (let alone moment) boundedness fails. *Where the erasure model differs from MP's noiseless rate-$R$:* MP assume a smooth rate-$R$ channel; here the rate is the random thinned process. The substitution $R\mapsto R(1-p)$ is justified by ergodic averaging of the **counting** bound (which is additive in delivered bits), not by treating the channel as smooth. The fluctuation of arrivals is a second-order effect that (A), not (R), governs.
...
> **Theorem D2★★.** If (R) and (A) hold **strictly** and [H-QC] holds, there exists a scheme (restoration-rate successive-refinement quantizer ∘ anytime/tree code over BEC$(p)$) achieving $m$-th-moment controlled set-invariance.
...
- **Step 2 — anytime/tree code over BEC.** Wrap the quantizer stream in a Sahai–Mitter / Sukhavasi–Hassibi tree code achieving anytime reliability exponent $\alpha$ up to the BEC$(p)$ anytime region; for low rate $\alpha\to\ln(1/p)=\alpha_{\mathrm{ch}}$.
- **Step 3 — composition.** Delivered slots contract by $e^{-R}$; a length-$b$ burst expands the top direction by $e^{b r^\star_{\mathrm{top}}}$. The stationary $m$-th moment is $\propto\sum_b(1-p)p^b e^{m r^\star_{\mathrm{top}}b}\cdot(\text{contraction})<\infty$ **iff** $p e^{m r^\star_{\mathrm{top}}}<1$ (A) and the average rate covers $h_R$ (R). Strict inequalities give a convergent geometric series ⇒ bounded moment. $\;\blacksquare$
...
### 2.5.2 Conjecture D2-Markov
> **Conjecture D2-Markov.** Under Gilbert–Elliott loss, $m$-th-moment set-invariance of nonlinear $f$ requires
> $$\rho\!\Big(P_e\cdot\mathrm{diag}\big(e^{\,m\,r^\star_{\mathrm{top},s}}\big)_{s\in\{G,B\}}\Big)<1,$$
> with $r^\star_{\mathrm{top},s}$ the (state-conditioned) top expansion rate during channel-state $s$. **Linear reduction:** $f=Ax$, $r^\star_{\mathrm{top},s}=\log\rho(A)$ ⇒ $\rho(P_e\,\rho(A)^m)<1$ = **You–Xie (2011)**. The i.i.d. case is the rank-one $P_e=\mathbf 1\pi^\top$ specialization, giving back $p\,e^{m r^\star_{\mathrm{top}}}<1$.
**To prove it:** combine You–Xie's Markov-jump second-moment Lyapunov analysis with Lemma C's volume counting, replacing the scalar gain by the per-state expansion; the obstacle is bounding the **product of non-commuting** state-dependent Jacobians over Markov-correlated burst windows (a matrix-multiplicative-ergodic / Oseledets argument under the Markov channel). **Closest technique:** Fang–Loparo Markov-jump linear systems + Furstenberg–Kesten products.
...
Circle map, $p<p_c$: implement successive-refinement quantizer at $R=\ln k+0.1$ + retransmission-based anytime code over the simulated BEC. **Prediction:** bounded $\mathbb E[e^2]$ at $T=10^4$ for $p<p_c$. **Falsification of D2★★:** instability with the optimal scheme at $p<p_c$.
